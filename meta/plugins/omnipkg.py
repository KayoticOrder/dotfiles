        ...
        if self._installCommand != "":
            ...
            for pkg in pkgList:
                if isinstance(pkg, str):
                    ...
                    existsInDict, exists, requireGUI, pkg, directive = (
                        self._getPkgNameFromDict(pkg)
                    )
                    if exists and not (requireGUI and not self._guiInstalled):
                        self._log.info(
                            "Found package: %s for %s - Installing" % (pkg, directive)
                        )
                else:
                    # invalid data
                ...
                if not existsInDict:
                    self._log.lowinfo(
                        "Skipping installation as no package specified for %s or %s"
                        % (self._packageManagerName, self._platformName)
                    )
                    ...
                    self._log.lowinfo(
                        "Skipping installation as package requires an installed GUI"
                    )
                else:
